module Measures
  ...
  class MATLoader
    ...
    def self.load_mat_exports(user, zip_file, out_dir, measure_details, vsac_user=nil, vsac_password=nil, overwrite_valuesets=true, cache=false, effectiveDate=nil, includeDraft=false, ticket_granting_ticket=nil)
      measure = nil
      if Measures::CqlLoader.mat_cql_export?(zip_file)
        measure = Measures::CqlLoader.load_mat_cql_exports(user, zip_file, out_dir, measure_details, vsac_user, vsac_password, overwrite_valuesets, cache, effectiveDate, includeDraft, ticket_granting_ticket)
