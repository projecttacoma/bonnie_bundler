...
class MeasureDiffTest < ActiveSupport::TestCase
  ...
  test "loading previous measure" do
    VCR.use_cassette("valid_vsac_response_158") do
      ...
      Measures::CqlLoader.extract_measures(@previous_measure, user, measure_details, { profile: APP_CONFIG['vsac']['default_profile'] }, get_ticket_granting_ticket).map {|measure| measure.save}
    end
  ...
  test "loading updated measure" do
    VCR.use_cassette("valid_vsac_response_158_update") do
      ...
      user.save
      measure_details = { 'episode_of_care'=> false }
      Measures::CqlLoader.extract_measures(@updated_measure, user, measure_details, { profile: APP_CONFIG['vsac']['default_profile'] }, get_ticket_granting_ticket).map {|measure| measure.save}
    end
    assert_equal 1, CqlMeasure.all.count
    measure = CqlMeasure.all.first
